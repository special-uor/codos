...
```{r}
## Start with a simple model to find the start points
lmod2 <- lm(log(vpd) ~ Tg + MI,# poly(Tg, 2) + poly(MI, 2),
           data = df_train)

model2 <- nls(vpd ~ a * exp(kTg * Tg - kMI * MI),
              df_train,
              start = list(a = exp(coef(lmod)[1]),
                           kTg = coef(lmod)[2],
                           kMI = coef(lmod)[3]),
...
```{r}
df5 <- data.frame(x = rep(seq(0, 6.6, 0.1), 7),
                  y = unlist(lapply(c(2.5, 7.5, 12.5, 17.5, 22.5, 27.5, 32.5),
                                    function(x) a[1] * exp(kTg[1] * x - kMI[1] * seq(0, 6.6, 0.1)))),
                  z = rep(c("0-5", "5-10", "10-15", "15-20", "20-25", "25-30", "30-35"),
                          each = length(seq(0, 6.6, 0.1))))
ggplot2::ggplot(df, ggplot2::aes(MI, vpd)) +
  ggplot2::geom_point(ggplot2::aes(color = tg), alpha = 0.5) +
...
```{r}
df6 <- data.frame(x = rep(seq(0, 35, 1), 5),
                  y = unlist(lapply(c(0.25, 0.75, 1.25, 1.75, 2.25),
                                    function(x) a[1] * exp(kTg[1] * seq(0, 35, 1) - kMI[1] * x))),
                  z = rep(c("0-0.5", "0.5-1", "1-1.5", "1.5-2", "2+"),
                          each = length(seq(0, 35, 1))))
ggplot2::ggplot(df, ggplot2::aes(Tg, vpd)) +
  ggplot2::geom_point(ggplot2::aes(color = mi), alpha = 0.5) +
...
### `0 < MI < 0.5`
```{r gs-tg-vpd-mi-0-0.5, eval = TRUE, echo = FALSE}
mi_min <- 0
mi_max <- 0.5
...
```
```{r gs-tg-vpd-mi-0.5-1, eval = TRUE, echo = FALSE}
mi_min <- 0.5
mi_max <- 1
filename <- paste0(path, 
                   "cru_ts4.04-clim-1961-1990-mi-",
...
```{r gs-tg-vpd-mi-1-1.5, eval = TRUE, echo = FALSE}
mi_min <- 1
mi_max <- 1.5
filename <- paste0(path, 
                   "cru_ts4.04-clim-1961-1990-mi-",
...
```{r gs-tg-vpd-mi-1.5-2, eval = TRUE, echo = FALSE}
mi_min <- 1.5
mi_max <- 2
filename <- paste0(path, 
                   "cru_ts4.04-clim-1961-1990-mi-",
...
```{r gs-tg-vpd-mi-2, eval = TRUE, echo = FALSE}
mi_min <- 2
filename <- paste0(path, 
                   "cru_ts4.04-clim-1961-1990-mi-",
                   mi_min, "-vpd.nc")
