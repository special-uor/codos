...
quinto_vicentino_clim <- quinto_vicentino_tmean %>%
...
quinto_vicentino_tmp <- with(quinto_vicentino_clim, codos::int_sin(Tjan, Tjul))
```

Fit a sinusoidal curve between the mean value for the coldest month, `T_jan =` `r round(quinto_vicentino_clim$Tjan, 4)`, and the warmest month, `T_jul =` `r round(quinto_vicentino_clim$Tjul, 4)`.

```{r modern-temp_fig, echo = FALSE, fig.height = 5}
codos::int_sin(quinto_vicentino_clim$Tmin, quinto_vicentino_clim$Tmax) %>%
  tibble::tibble(x = seq_len(365), y = .) %>%
  ggplot2::ggplot(ggplot2::aes(x, y)) +
      ggplot2::geom_line() +
      ggplot2::geom_point(data = tibble::tibble(x = 1, y = quinto_vicentino_clim$Tjan),
                          ggplot2::aes(x, y, colour = "Tjan")) +
      ggplot2::geom_point(data = tibble::tibble(x = 365 / 2, y = quinto_vicentino_clim$Tjul),
...
tibble::tibble(x = rep(fimon$age_cal_yr_BP, 4),
               y = c(fimon$Tjan,
                     fimon$Tjul,
                     fimon$Tgs,
...
fimon2 <- tibble::tibble(age_calBP = fimon$age_cal_yr_BP,
                         ...
                         palaeo_temp = fimon$Tgs,
                         modern_co2 = modern_co2,
                         modern_temp = mean(quinto_vicentino_tmp),
                         recon_mi = fimon$MI,
...
```{r find-corrected-mi}
corr_mi <- fimon2 %$%
  codos::corrected_mi(Tc0 = modern_temp,
                      Tc1 = palaeo_temp,
                      MI = recon_mi,
                      ca0 = modern_co2,
                      ca1 = palaeo_co2)
...
fimon2$corr_Pann <- fimon2$recon_Pann * mi_ratio
```
...
fimon_modern <- quinto_vicentino_clim
```
...
```{r anomaly-ex1_fig, fig.height = 5, echo = FALSE}
...
- Temperature:  [`CRU TS 4.04`] $+ \text{Tmp}_\text{anomalies}$
...
```{r, echo = FALSE}
fimon_corrected_pr <- purrr::map_dbl(seq_along(fimon_pet_ultimate),
                                     ~sum(fimon_pet_ultimate[.x][[1]], na.rm = TRUE) * fimon2$corr_mi[.x])
# fimon_corrected_pr_ultimate <- fimon_corrected_pr
...
fimon %>%
  ...
  dplyr::mutate(`corr MI` = fimon2$corr_mi,
                `corr Pann (MI ratio)` = fimon2$corr_Pann,
                `corr Pann (Tmp + Cld anomalies & Orb. Par.)` = fimon_corrected_pr) %>%
  dplyr::slice(1:12) %>%
  knitr::kable() # "pandoc", col.names = c(names(.)[1:5], "corrected MI", "corrected Pann (MI ratio)", "corrected Pann (Tmp anomalies)"))
```
...
tibble::tibble(x = rep(fimon2$age_calBP, 2),
               ...
               palaeo_temp = rep(fimon2$palaeo_temp / 5, 2),
  ...
  ggplot2::geom_line(ggplot2::aes(x, palaeo_temp, colour = "Temp")) +
...
#### Reconstructed vs corrected `Pann`: Past CO2 calculated using `mean`
...
```{r, echo = FALSE}
# fimon2$corr_Pann <- fimon2$recon_Pann * mi_ratio
fimon2$corr_Pann <- fimon_corrected_pr_ultimate
fimon2 %>% 
  readr::write_excel_csv(file = "lago-di-fimon-with-corrected-mi-and-pann.csv", 
                         na = "")
