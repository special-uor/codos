...
codos::int_sin(padul$Tmin[9], padul$Tmax[9]) %>%
  tibble::tibble(x = seq_len(365), y = .) %>%
  ggplot2::ggplot(ggplot2::aes(x, y)) +
      ggplot2::geom_line() +
      ggplot2::geom_point(data = tibble::tibble(x = 1, y = padul$T_djf[9]),
                          ggplot2::aes(x, y, colour = "T_djf")) +
      ggplot2::geom_point(data = tibble::tibble(x = 365 / 2, y = padul$T_jja[9]),
                          ggplot2::aes(x, y, colour = "T_jja")) +
      ggplot2::scale_colour_manual(name = "Recon. \nTemp.",
                                   values = c("#0080ff","#ff3333")) +
      ggplot2::labs(x = "[days]",
                    y = "Temp [°C]",
                    title = paste0(padul$`Age (cal yr BP)`[9], " cal yr BP")) +
      ggplot2::theme_bw()
```
...
padul %>%
  dplyr::slice(179) %>%
  rbind(padul_modern %>%
          dplyr::mutate(`Age (cal yr BP)` = NA, .before = 1),
        .) %>%
...
codos::int_sin(padul$Tmin[179] - padul_modern$Tmin, 
               padul$Tmax[179] - padul_modern$Tmax) %>%
  tibble::tibble(x = seq_len(365), y = .) %>%
  ggplot2::ggplot(ggplot2::aes(x, y)) +
      ggplot2::geom_line() +
      ggplot2::geom_point(data = tibble::tibble(x = 1, y = padul$T_djf[179] - padul_modern$T_djf),
                          ggplot2::aes(x, y, colour = "dT_djf")) +
      ggplot2::geom_point(data = tibble::tibble(x = 365 / 2, y = padul$T_jja[179] - padul_modern$T_jja),
...
```{r, fig.height = 5, echo = FALSE}
codos::int_sin(padul$Tmin[274] - padul_modern$Tmin, 
               padul$Tmax[274] - padul_modern$Tmax) %>%
  tibble::tibble(x = seq_len(365), y = .) %>%
  ggplot2::ggplot(ggplot2::aes(x, y)) +
      ggplot2::geom_line() +
      ggplot2::geom_point(data = tibble::tibble(x = 1, y = padul$T_djf[274] - padul_modern$T_djf),
                          ggplot2::aes(x, y, colour = "dT_djf")) +
      ggplot2::geom_point(data = tibble::tibble(x = 365 / 2, y = padul$T_jja[274] - padul_modern$T_jja),
                          ggplot2::aes(x, y, colour = "dT_jja")) +
      ggplot2::scale_colour_manual(name = "Recon. \nTemp.",
                                   values = c("#0080ff","#ff3333")) +
      ggplot2::labs(x = "[days]",
