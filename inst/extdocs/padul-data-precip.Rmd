...
padul_corrected_pr <- purrr::map_dbl(seq_along(padul_pet_tmp),
                                     ~sum(padul_pet_tmp[.x][[1]], na.rm = TRUE) * padul2$corrected_mi[.x])
```
...
padul %>%
  dplyr::select(1:3) %>%
  dplyr::mutate(corrected_mi = padul2$corrected_mi,
                `corrected_P_ann (MI ratio)` = padul2$corrected_P_ann,
                `corrected_P_ann (Tmp anomalies)` = padul_corrected_pr) %>%
...
```{r}
padul_cld_corrected_mi <- padul2$corrected_mi %>%
  purrr::map_dbl(codos::cld)
```
...
```{r, echo = FALSE}
ref_pdul_cld_corrected_mi <- padul_cld_corrected_mi %>%
  .[ref_idx] %>%
  mean()
```
...
padul_cld_anomalies <- seq_len(nrow(padul)) %>%
  purrr::map_dbl(~padul_cld_corrected_mi[.x] - ref_pdul_cld_corrected_mi)
```
...
year <- 1961
padul_sf2 <- 1 - (padul_cld_corrected_mi + padul_cld_anomalies) / 100

oplan <- future::plan(future::multisession, workers = 8)
{p <- progressr::progressor(steps = length(padul_cld_corrected_mi))
padul_pet_cld <- 
  seq_along(padul_sf2) %>%
  furrr::future_map(function(k) {
    p()
...
padul_corrected_pr_cld <- 
  purrr::map_dbl(seq_along(padul_pet_cld),
                 ~sum(padul_pet_cld[.x][[1]], na.rm = TRUE) * padul2$corrected_mi[.x])
```
...
padul_corrected_pr_tmp_cld <- purrr::map_dbl(seq_along(padul_pet_tmp_cld),
                                         ~sum(padul_pet_tmp_cld[.x][[1]], na.rm = TRUE) * padul2$corrected_mi[.x])
```

```{r, echo = FALSE}
padul2 %>%
  dplyr::mutate(corrected_P_ann = padul_corrected_pr_tmp_cld) %>%
...
padul_corrected_pr_orb_params <- 
  purrr::map_dbl(seq_along(padul_pet_orb_params),
                 ~sum(padul_pet_orb_params[.x][[1]], na.rm = TRUE) * padul2$corrected_mi[.x])
```
...
padul_corrected_pr_ultimate <- purrr::map_dbl(seq_along(padul_pet_ultimate),
                                     ~sum(padul_pet_ultimate[.x][[1]], na.rm = TRUE) * padul2$corrected_mi[.x])
```
...
ggplot2::ggplot(ggplot2::aes(x, y, colour = var)) +
  ggplot2::geom_line() +
  ggplot2::scale_colour_brewer(name = "Precipitation", palette = "Set2") + 
  ggplot2::scale_x_continuous(breaks = scales::pretty_breaks(15)) +
  ggplot2::scale_y_continuous(breaks = scales::pretty_breaks(15)) +
...
tibble::tibble(age_cal_yr_BP = padul$age_cal_yr_BP,
               ...
               corrected_mi = padul2$corrected_mi,
               recon_P_ann = padul$P_ann,
               corrected_P_ann_MI_ratio = padul2$corrected_P_ann,
               corrected_P_ann_Tmp_anomalies = padul_corrected_pr,
               corrected_P_ann_Cld_anomalies = padul_corrected_pr_cld,
               corrected_P_ann_Orbital_params = padul_corrected_pr_orb_params,
               corrected_P_ann_Tmp_Cld_anomalies = padul_corrected_pr_tmp_cld,
               corrected_P_ann_All = padul_corrected_pr_ultimate) %>%
